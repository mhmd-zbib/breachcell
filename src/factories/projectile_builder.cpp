...
ProjectileBuilder::ProjectileBuilder(EntityManager* entityManager) : entityManager(entityManager) {
}
...
std::uint32_t ProjectileBuilder::build() {
  ...
  std::cout << "ProjectileBuilder: Created projectile entity " << entityId << " at (" << positionX
            << ", " << positionY << ") with velocity (" << velocityX << ", " << velocityY
            << ") and ownerId " << ownerId << std::endl;
  return entityId;
}
