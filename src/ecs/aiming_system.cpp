...
void AimingSystem::update(std::uint32_t playerEntityId, float mouseX, float mouseY,
  ...
  playerAim->aimAngle +=
      (newAimAngle - playerAim->aimAngle) * std::clamp(aimLerpSpeed * 0.016f, 0.0f, 1.0f);
  float velocityMagnitude = playerVelocity ? MathUtils::vectorMagnitude(playerVelocity->velocityX,
                                                                        playerVelocity->velocityY)
                                           : 0.0f;
...
void AimingSystem::update(std::uint32_t playerEntityId, EntityManager* entityManager,
                          CameraService* cameraService, InputHandler* inputHandler) {
  int mouseX = 0;
  int mouseY = 0;
  SDL_GetMouseState(&mouseX, &mouseY);
  update(playerEntityId, static_cast<float>(mouseX), static_cast<float>(mouseY), entityManager,
         cameraService, inputHandler);
}
...
float AimingSystem::getShootConeHalfAngle(std::uint32_t  playerEntityId,
