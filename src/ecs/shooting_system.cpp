...
ShootingSystem::ShootingSystem(EntityManager*     entityManagerPtr,
                               ProjectileFactory* projectileFactoryPtr) :
    entityManager(entityManagerPtr), projectileFactory(projectileFactoryPtr) {
}

void ShootingSystem::update(std::uint32_t shooterEntityId) {
  if (!entityManager || !projectileFactory)
    return;
  ...
  bool                    mouseButtonCurrentlyPressed =
  ...
  if (mouseButtonCurrentlyPressed && !mouseButtonPreviouslyPressed &&
    ...
    TransformComponent* shooterTransformComponent =
        entityManager->getTransformComponent(shooterEntityId);
    ...
    projectileFactory->createProjectile(projectileSpawnX, projectileSpawnY, projectileVelocityX,
                                        projectileVelocityY, projectileWidth, projectileHeight,
                                        projectileLifetime, projectileTextureId, shooterEntityId);
