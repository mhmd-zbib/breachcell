...
void MovementSystem::update(float deltaTime, std::uint32_t playerEntityId)
{
  ...
  if (velocity && transform)
  {
    ...
    float speed = inputHandler.isKeyPressed(SDLK_LSHIFT)
                  || inputHandler.isKeyPressed(SDLK_RSHIFT) ? SLOW_SPEED : NORMAL_SPEED;
    ...
    float magnitude = std::sqrt(velocity->velocityX * velocity->velocityX + velocity->velocityY *
                                velocity->velocityY);
  ...
  for (std::uint32_t entityId = 1; entityId < EntityManager::MAX_ENTITY_ID; ++entityId)
  {
    ...
    if (projectile && transform && velocity)
    {
      ...
      std::cout << "Projectile entity " << entityId << " framesAlive=" << projectile->framesAlive <<
                " position=(" << transform->positionX << ", " << transform->positionY << ")" << std::endl;
